 ...
 * 功能说明：
 * 1. 实现客户的增删改查
 * 2. 客户编码由后端自动生成（使用 CodeGeneratorService）
 * 3. 提供客户批量查询Map接口
 * 4. 删除前校验是否有关联出库单
 *
 * @author laby
...
public class CustomerServiceImpl implements CustomerService {
    ...
    public Long createCustomer(CustomerSaveReqVO createReqVO) {
        // 1. 生成客户编码（由后端自动生成）
        String customerCode = codeGeneratorService.generateCode(CodePrefixEnum.CUSTOMER);
        ...
        log.info("[createCustomer] 创建客户成功，客户编码：{}, ID：{}", customer.getCustomerCode(), customer.getId());
    ...
    public void updateCustomer(CustomerSaveReqVO updateReqVO) {
        ...
        // 2. 更新数据库（客户编码不可修改，由后端生成）
        CustomerDO updateObj = CustomerConvert.INSTANCE.convert(updateReqVO);
        customerMapper.updateById(updateObj);
        
        log.info("[updateCustomer] 更新客户成功，ID：{}", updateReqVO.getId());
    }
