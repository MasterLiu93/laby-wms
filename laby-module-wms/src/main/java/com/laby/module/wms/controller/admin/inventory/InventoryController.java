...
public class InventoryController {
    ...
    public CommonResult<PageResult<InventoryRespVO>> getInventoryPage(@Valid InventoryPageReqVO pageReqVO) {
        ...
        // 查询库区信息
        List<Long> areaIds = locationMap.values().stream()
                .map(WarehouseLocationDO::getAreaId)
                .filter(java.util.Objects::nonNull)
                .distinct()
                .collect(Collectors.toList());
        Map<Long, String> areaMap = warehouseAreaService.getWarehouseAreaMap(areaIds).entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getAreaName()));
        
        // 转换并填充关联字段
        PageResult<InventoryRespVO> result = InventoryConvert.INSTANCE.convertPage(pageResult, warehouseMap, locationMap, goodsMap, areaMap);
