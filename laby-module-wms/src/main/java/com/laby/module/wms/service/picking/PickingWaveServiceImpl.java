...
public class PickingWaveServiceImpl implements PickingWaveService {
    ...
    @Resource
    private com.laby.module.wms.dal.mysql.outbound.OutboundItemMapper outboundItemMapper;

    @Resource
    private com.laby.module.wms.dal.mysql.picking.PickingTaskMapper pickingTaskMapper;

    @Resource
    private com.laby.module.wms.service.goods.GoodsService goodsService;

    @Resource
    private com.laby.module.wms.service.warehouse.WarehouseLocationService warehouseLocationService;
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int generatePickingTasks(Long waveId) {
        // 1. 校验波次是否存在
        PickingWaveDO wave = validatePickingWaveExists(waveId);

        // 2. 校验状态（只有待分配状态可以生成任务）
        if (!PickingWaveStatusEnum.PENDING_ASSIGN.getStatus().equals(wave.getStatus())) {
            throw exception(PICKING_WAVE_NOT_ALLOW_GENERATE_TASK);
        }

        // 3. 查询波次关联的出库单
        List<PickingWaveOrderDO> waveOrders = pickingWaveOrderMapper.selectList(
                PickingWaveOrderDO::getWaveId, waveId);
        if (CollUtil.isEmpty(waveOrders)) {
            throw exception(PICKING_WAVE_OUTBOUND_EMPTY);
        }

        // 4. 为每个出库单的每个明细创建拣货任务
        int taskCount = 0;
        for (PickingWaveOrderDO waveOrder : waveOrders) {
            // 4.1 查询出库单明细
            List<com.laby.module.wms.dal.dataobject.outbound.OutboundItemDO> outboundItems = 
                    outboundItemMapper.selectListByOutboundId(waveOrder.getOutboundId());
            ...
            // 4.2 查询出库单信息（获取仓库ID）
            OutboundDO outbound = outboundMapper.selectById(waveOrder.getOutboundId());
            if (outbound == null) {
                continue;
            }

            // 4.3 批量查询商品信息
            List<Long> goodsIds = outboundItems.stream()
                    .map(OutboundItemDO::getGoodsId)
                    .distinct()
                    .collect(Collectors.toList());
            Map<Long, GoodsDO> goodsMap = goodsService.getGoodsMap(goodsIds);

            // 4.4 批量查询库位信息
            List<Long> locationIds = outboundItems.stream()
                    .map(OutboundItemDO::getLocationId)
                    .filter(Objects::nonNull)
                    .distinct()
                    .collect(Collectors.toList());
            Map<Long, WarehouseLocationDO> locationMap = 
                    warehouseLocationService.getWarehouseLocationMap(locationIds);

            // 4.5 为每个明细创建拣货任务
            int sortOrder = 1;
            for (OutboundItemDO item : outboundItems) {
                PickingTaskDO task = new PickingTaskDO();
                task.setTaskNo(generateTaskNo());
                task.setWaveId(waveId);
                task.setWaveNo(wave.getWaveNo());
                task.setOutboundId(outbound.getId());
                task.setOutboundNo(outbound.getOutboundNo());
                task.setWarehouseId(outbound.getWarehouseId());
                task.setGoodsId(item.getGoodsId());
                task.setBatchNo(item.getBatchNo());
                task.setLocationId(item.getLocationId());
                task.setPlanQuantity(item.getPlanQuantity());
                task.setActualQuantity(BigDecimal.ZERO);
                task.setSortOrder(sortOrder++);
                task.setStatus(PickingTaskStatusEnum.PENDING.getStatus());

                // 填充冗余字段：商品信息
                GoodsDO goods = goodsMap.get(item.getGoodsId());
                if (goods != null) {
                    task.setSkuCode(goods.getSkuCode());
                    task.setGoodsName(goods.getGoodsName());
                }

                // 填充冗余字段：库位信息
                if (item.getLocationId() != null) {
                    WarehouseLocationDO location = locationMap.get(item.getLocationId());
                    if (location != null) {
        ...
        log.info("[拣货波次] 生成拣货任务成功，波次ID：{}，任务数量：{}", waveId, taskCount);
        return taskCount;
    }
    ...
    private String generateTaskNo() {
        ...
        // 查询当天最大任务号
        String prefix = "PICK-" + date + "-";
        com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<PickingTaskDO> wrapper = 
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<>();
        wrapper.like(PickingTaskDO::getTaskNo, prefix)
                .orderByDesc(PickingTaskDO::getTaskNo)
                .last("LIMIT 1");
        List<PickingTaskDO> todayTasks = pickingTaskMapper.selectList(wrapper);
        
        int sequence = 1;
        if (!todayTasks.isEmpty()) {
            String lastTaskNo = todayTasks.get(0).getTaskNo();
            try {
                String sequenceStr = lastTaskNo.substring(prefix.length());
                sequence = Integer.parseInt(sequenceStr) + 1;
            } catch (Exception e) {
                log.warn("[生成任务编号] 解析上一个任务编号失败：{}", lastTaskNo, e);
            }
        }
    ...
    private List<OutboundDO> queryOutboundsForWave(Long warehouseId) {
        ...
        List<OutboundDO> allOutbounds = outboundMapper.selectList(
                ...
                OutboundDO::getStatus, OutboundStatusEnum.APPROVED.getStatus()
        );

        // 过滤掉已经在波次中的出库单（返回可变列表）
