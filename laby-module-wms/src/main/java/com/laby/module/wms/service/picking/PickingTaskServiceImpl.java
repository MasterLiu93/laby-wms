...
public class PickingTaskServiceImpl implements PickingTaskService {
    ...
    public void executePicking(PickingTaskPickReqVO pickReqVO) {
        ...
        // 7. 扣减库存（实际拣货数量）
        if (pickReqVO.getActualQuantity().compareTo(BigDecimal.ZERO) > 0) {
            reduceInventory(task, pickReqVO.getActualQuantity());
        }

        // 8. 如果拣货完成（无异常），更新出库单的拣货数量
        if (pickReqVO.getExceptionType() == null) {
        ...
        // 9. checkAndUpdatePickingStatus 方法会自动检查所有拣货任务是否完成
        //    如果完成则更新出库单状态为"待发货"
    }
    ...
    @Override
    public void fillPickingTaskDetail(PickingTaskRespVO vo) {
        if (vo == null) {
            return;
        }
        ...
        if (vo.getWarehouseId() != null) {
            WarehouseDO warehouse = warehouseService.getWarehouse(vo.getWarehouseId());
            if (warehouse != null) {
        ...
        if (vo.getLocationId() != null) {
            WarehouseLocationDO location = warehouseLocationService.getWarehouseLocation(vo.getLocationId());
            if (location != null) {
                vo.setLocationCode(location.getLocationCode());
                vo.setRowNo(location.getRowNo());
                vo.setColumnNo(location.getColumnNo());
                vo.setLayerNo(location.getLayerNo());
                
                // 填充库区信息
                if (location.getAreaId() != null) {
                    vo.setAreaId(location.getAreaId());
                    WarehouseAreaDO area = warehouseAreaService.getWarehouseArea(location.getAreaId());
                    if (area != null) {
     ...
     * @param quantity 拣货数量
     */
    private void reduceInventory(PickingTaskDO task, BigDecimal quantity) {
        // 1. 查询库存记录
        LambdaQueryWrapper<InventoryDO> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(InventoryDO::getWarehouseId, task.getWarehouseId())
               .eq(InventoryDO::getGoodsId, task.getGoodsId())
               .eq(InventoryDO::getLocationId, task.getLocationId())
               .eq(InventoryDO::getBatchNo, task.getBatchNo());
        ...
        updateObj.setQuantity(inventory.getQuantity().subtract(quantity));
        ...
        inventoryLog.setWarehouseId(task.getWarehouseId());
        inventoryLog.setGoodsId(task.getGoodsId());
        inventoryLog.setLocationId(task.getLocationId());
        inventoryLog.setBatchNo(task.getBatchNo());
        inventoryLog.setOperationType("PICKING"); // 拣货出库
        inventoryLog.setQuantityChange(quantity.negate()); // 负数表示减少
        inventoryLog.setQuantityBefore(quantityBefore);
        inventoryLog.setQuantityAfter(quantityBefore.subtract(quantity));
        inventoryLog.setBusinessType("PICKING_TASK");
        inventoryLog.setBusinessNo(task.getTaskNo());
        inventoryLog.setRemark("拣货任务扣减库存");
        inventoryLogMapper.insert(inventoryLog);

        log.info("[拣货任务] 扣减库存成功，任务号：{}，商品ID：{}，库位ID：{}，扣减数量：{}，剩余库存：{}", 
                task.getTaskNo(), task.getGoodsId(), task.getLocationId(), quantity, quantityBefore.subtract(quantity));
    }
