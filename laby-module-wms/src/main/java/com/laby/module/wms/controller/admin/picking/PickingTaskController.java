...
public class PickingTaskController {
    ...
    public CommonResult<PageResult<PickingTaskRespVO>> getPickingTaskPage(@Valid PickingTaskPageReqVO pageReqVO) {
        ...
        PageResult<PickingTaskRespVO> voPageResult = PickingTaskConvert.INSTANCE.convertPage(pageResult);
        
        // 填充关联信息：仓库、库区、库位详细信息
        if (cn.hutool.core.collection.CollUtil.isNotEmpty(voPageResult.getList())) {
            voPageResult.getList().forEach(vo -> pickingTaskService.fillPickingTaskDetail(vo));
        }
