...
public class WarehouseAreaServiceImpl implements WarehouseAreaService {
    ...
    public Long createWarehouseArea(WarehouseAreaSaveReqVO createReqVO) {
        ...
        // 2. 生成库区编码
        String areaCode = codeGeneratorService.generateCode(CodePrefixEnum.WAREHOUSE_AREA);
        ...
        log.info("[createWarehouseArea] 创建库区成功，库区编码：{}, ID：{}", warehouseArea.getAreaCode(), warehouseArea.getId());
     ...
     * 1. 校验库区是否存在
     * 2. 校验仓库是否存在
     * 3. 使用 MapStruct 转换 VO 为 DO（编码不可修改，由后端自动生成）
