...
public class PickingWaveController {
    ...
    @Parameter(name = "id", description = "波次ID", required = true, example = "1")
    @PreAuthorize("@ss.hasPermission('wms:picking-wave:create')")
    public CommonResult<Integer> generateTasks(@RequestParam("id") Long id) {
        int taskCount = pickingWaveService.generatePickingTasks(id);
     ...
     * @param waveType 波次类型（BATCH/ZONE/SINGLE）
     * @return 生成的波次ID列表
     */
    @PostMapping("/generate-waves")
    @Operation(summary = "自动生成拣货波次")
    @PreAuthorize("@ss.hasPermission('wms:picking-wave:create')")
    public CommonResult<List<Long>> generateWaves(
            @RequestParam("warehouseId") Long warehouseId,
            @RequestParam(value = "waveType", required = false, defaultValue = "BATCH") String waveType) {
        List<Long> waveIds = pickingWaveService.generatePickingWaves(warehouseId, waveType);
        return success(waveIds);
    }
}
