...
public class WarehouseLocationController {
    ...
    @Operation(summary = "获得指定仓库的所有库位", description = "用于入库单/出库单选择库位")
    @Parameter(name = "warehouseId", description = "仓库ID", required = true, example = "1")
    @PreAuthorize("@ss.hasPermission('wms:location:query')")
    public CommonResult<List<WarehouseLocationSimpleRespVO>> getWarehouseLocationListByWarehouseId(@RequestParam("warehouseId") Long warehouseId) {
        List<WarehouseLocationDO> list = warehouseLocationService.getWarehouseLocationListByWarehouseId(warehouseId);
