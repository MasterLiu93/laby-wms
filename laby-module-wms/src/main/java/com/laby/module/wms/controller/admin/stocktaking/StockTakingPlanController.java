...
public class StockTakingPlanController {
     ...
     * @return 生成的盘点单数量
     */
    @PostMapping("/generate-stock-taking")
    @Operation(summary = "生成盘点单")
    @Parameter(name = "id", description = "盘点计划ID", required = true, example = "1")
    @PreAuthorize("@ss.hasPermission('wms:stock-taking-plan:generate')")
    public CommonResult<Integer> generateStockTaking(@RequestParam("id") Long id) {
        int count = stockTakingPlanService.generateStockTaking(id);
