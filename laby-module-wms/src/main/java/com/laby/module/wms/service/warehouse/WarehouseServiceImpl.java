...
public class WarehouseServiceImpl implements WarehouseService {
    ...
    public Long createWarehouse(WarehouseSaveReqVO createReqVO) {
        // 1. 生成仓库编码
        String warehouseCode = codeGeneratorService.generateCode(CodePrefixEnum.WAREHOUSE);
        ...
        log.info("[createWarehouse] 创建仓库成功，仓库编码：{}, ID：{}", warehouse.getWarehouseCode(), warehouse.getId());
     ...
     * 实现步骤：
     * 1. 校验仓库是否存在
     * 2. 使用 MapStruct 转换 VO 为 DO（编码不可修改，由后端自动生成）
     * 3. 更新数据库（MyBatis Plus 自动更新修改时间等）
     */
    @Override
