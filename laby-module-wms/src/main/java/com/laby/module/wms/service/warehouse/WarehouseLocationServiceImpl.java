...
public class WarehouseLocationServiceImpl implements WarehouseLocationService {
    ...
    public Long createWarehouseLocation(WarehouseLocationSaveReqVO createReqVO) {
        ...
        // 3. 生成库位编码
        String locationCode = codeGeneratorService.generateCode(CodePrefixEnum.WAREHOUSE_LOCATION);

        // 4. 转换并设置编码
        WarehouseLocationDO warehouseLocation = WarehouseLocationConvert.INSTANCE.convert(createReqVO);
        warehouseLocation.setLocationCode(locationCode);
        
        // 5. 插入数据库
        warehouseLocationMapper.insert(warehouseLocation);
        
        log.info("[createWarehouseLocation] 创建库位成功，库位编码：{}, ID：{}", warehouseLocation.getLocationCode(), warehouseLocation.getId());
     ...
     * 2. 校验仓库是否存在
     * 3. 校验库区是否存在
     * 4. 使用 MapStruct 转换 VO 为 DO（编码不可修改，由后端自动生成）
    ...
    public void updateWarehouseLocation(WarehouseLocationSaveReqVO updateReqVO) {
        ...
        log.info("[updateWarehouseLocation] 更新库位成功，ID：{}", updateReqVO.getId());
    }
     ...
     */
    @Override
    public List<WarehouseLocationDO> getWarehouseLocationListByWarehouseId(Long warehouseId) {
        return warehouseLocationMapper.selectList(WarehouseLocationDO::getWarehouseId, warehouseId);
    }
