 ...
 * @returns 生成的任务数量
 */
export const generatePickingTasks = (id: number) => {
  return request.post({ url: `/wms/picking-wave/generate-tasks?id=${id}` })
}
 ...
 * @returns 生成的波次ID列表
 */
export const generatePickingWaves = (warehouseId: number, waveType: string) => {
  return request.post({
    url: '/wms/picking-wave/generate-waves',
    params: { warehouseId, waveType }
  })
}
