...
export interface OutboundVO {
  ...
  outboundType: number // 出库类型：1-销售出库，2-退货出库，3-调拨出库，4-其他出库
...
export const startPicking = (id: number) => {
  return request.put({ url: '/wms/outbound/start-picking', params: { id } })
}
...
export const cancelOutbound = (id: number) => {
  return request.put({ url: '/wms/outbound/cancel', params: { id } })
}
