<template>
  <el-form
    ...
    :rules="LoginRules"
    class="login-form modern-login-form"
    label-position="top"
    label-width="120px"
    size="default"
  >
...
const doSocialLogin = async (type: number) => {
  if (type === 0) {
    message.error(t('login.socialMethodNotConfigured'))
  } else {
    loginLoading.value = true
    if (loginData.tenantEnable === 'true') {
      ...
      if (!authUtil.getTenantId()) {
        try {
          const data = await message.prompt(t('login.inputTenantName'), t('common.reminder'))
