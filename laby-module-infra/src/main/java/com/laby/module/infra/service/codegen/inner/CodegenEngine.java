package com.laby.module.infra.service.codegen.inner;
...
import com.laby.framework.apilog.core.enums.OperateTypeEnum;
...
import com.laby.framework.common.util.date.DateUtils;
import com.laby.framework.common.util.date.LocalDateTimeUtils;
import com.laby.framework.common.util.object.BeanUtils;
import com.laby.framework.common.util.object.ObjectUtils;
import com.laby.framework.common.util.string.StrUtils;
import com.laby.framework.excel.core.annotations.DictFormat;
import com.laby.framework.excel.core.convert.DictConvert;
...
import com.laby.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.laby.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.laby.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.laby.module.infra.enums.codegen.CodegenSceneEnum;
import com.laby.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.laby.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.laby.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 laby-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 laby-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.laby.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/laby-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/laby-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "laby-module-${table.moduleName}/" + // 顶级模块
                "laby-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "laby-module-${table.moduleName}/" + // 顶级模块
                "laby-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "laby-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "laby-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
